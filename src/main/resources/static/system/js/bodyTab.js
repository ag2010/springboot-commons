...
layui.define(["element","jquery"],function(exports){
    ...
    Tab.prototype.navBar = function(strData){
        ...
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].childMenu != undefined && data[i].childMenu.length > 0){
                ...
                ulHtml += '<cite>'+data[i].menu_name+'</cite>';
                ulHtml += '<span class="layui-nav-more"></span>';
                ulHtml += '</a>';
                ulHtml += '<dl class="layui-nav-child">';
                for(var j=0;j<data[i].childMenu.length;j++){
                    if(data[i].childMenu[j].target == "_blank"){
                        ulHtml += '<dd><a data-url="'+data[i].childMenu[j].link_url+'" target="'+data[i].childMenu[j].target+'">';
                    }else{
                        ulHtml += '<dd><a data-url="'+data[i].childMenu[j].link_url+'">';
                    }
                    if(data[i].childMenu[j].iconCls != undefined && data[i].childMenu[j].iconCls != ''){
                        if(data[i].childMenu[j].iconCls.indexOf("icon-") != -1){
                            ulHtml += '<i class="seraph '+data[i].childMenu[j].iconCls+'" data-icon="'+data[i].childMenu[j].iconCls+'"></i>';
                        }else{
                            ulHtml += '<i class="layui-icon" data-icon="'+data[i].childMenu[j].iconCls+'">'+data[i].childMenu[j].iconCls+'</i>';
                        }
                    }
                    ulHtml += '<cite>'+data[i].childMenu[j].menu_name+'</cite></a></dd>';
                }
                ulHtml += "</dl>";
            }else{
                if(data[i].target == "_blank"){
                    ulHtml += '<a data-url="'+data[i].link_url+'" target="'+data[i].target+'">';
                }else{
                    ulHtml += '<a data-url="'+data[i].link_url+'">';
                }
                ...
                ulHtml += '<cite>'+data[i].menu_name+'</cite></a>';
            }
            ulHtml += '</li>';
        }
        return ulHtml;
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="page/main.html"><i class="layui-icon" data-icon=""></i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height());
